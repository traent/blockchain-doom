...
import { doomWad } from './doom1.wad.base64.js';
import { doomConfig } from './default.cfg.js';
import { wasmBinaryFileBase64 } from './websockets-doom.wasm.base64.js';
import { handleFrame } from './handle-frame.js';

export const logQueue = [];
let logHandler = typeof consoleWriteLine !== 'undefined' ? consoleWriteLine : undefined;
export function setLogHandler(lh) {
  logHandler = lh;
}
function emitLog(l) {
  logQueue.push(l);
  if (logHandler) {
    while (logQueue.length > 0) {
      logHandler(logQueue.shift());
    }
  }
}

let frameHandler = typeof handleFrame !== 'undefined' ? handleFrame : undefined;
export function setFrameHandler(fh) {
  frameHandler = fh;
}

export let stats = [];

let endGameHandler = (x) => console.log(x);
export function setEndGameHandler(handler) {
  endGameHandler = handler;
}

let inputEvents = [];
export function setInputEvents(i) {
  inputEvents = [...i];
  removeRunDependency("input-events");
}

import { inputs } from './inputs.js';
// setInputEvents(inputs.events);

export var digests = [];

function heapHash() {
  let r = 0x100000000;
  for (const x of HEAPU32 ?? []) {
    r ^= x;
  }

  return `0x${r.toString(16).substring(1)}`;
}
...
var dateNow = () => {
  if (!ended) {
    const state = HEAP32 ? {
      health: HEAP32[236114],
      armor: HEAP32[236114 + 1],
      episode: HEAP32[236562],
      mission: HEAP32[236092],
      skill: HEAP32[236393],
    } : {};

    const stateString = JSON.stringify(state);
    if (stateString !== stateSnapshot) {
      stateSnapshot = stateString;
      stats.push({ ...state, timestamp: tick });
    }

    // end game
    ended = state.mission === 2 ||
      (state.health === 0 && state.mission === 1);
    if (ended) {
      const digest = heapHash();
  ...
  while (inputEvents[eventIndex] && inputEvents[eventIndex].timeStamp === tick) {
    dispatchEvent(inputEvents[eventIndex++]);
  }
...
var devicePixelRatio = undefined;
...
import { minDocument } from './document.js';
...
if (typeof window !== 'undefined') {
  ...
  // debug helpers
  console = {
    log: (...args) => emitLog(`${args.join(' ')}`),
    warn: (...args) => emitLog(`WARN: ${args.join(' ')}`),
    error: (...args) => emitLog(`ERR: ${args.join(' ')}`),
  };

  globalThis.setTimeout = (callback) => { Promise.resolve().then(callback); };
  globalThis.document = minDocument;
  globalThis.WebGLRenderingContext = class { };
  const canvas = document.createElement('canvas');
  canvas.id = 'canvas';
  canvas.getBoundingClientRect = () => screen;
  canvas.getContext = (type) => type !== '2d'
    ? null
    : {
      createImageData: (w, h) => new ImageData(w, h),
      putImageData: (image) => frameHandler(image, tick),
    };
...
const rewireAddEventListener = (target) => {
  target.originalAddEventListener = target.addEventListener;
  target.addEventListener = (type, handler, useCapture) => {
    if (!eventListeners[type]) {
      target.originalAddEventListener(type, (e) => {
        const event = serializeEvent(e);
  ...
  target.removeEventListener = (type, handler) => {
    eventListeners[type] = [...eventListeners[type]].filter((x) => x !== handler);
  };
};
...
function dispatchEvent(evt) {
  void Promise.resolve().then(() => {
    ...
    for (const handler of eventListeners[evt.type] ?? []) {
      handler(evt);
    }
  });
}
...
rewireAddEventListener(canvas);
rewireAddEventListener(document);
if (typeof window !== 'undefined') {
  rewireAddEventListener(window);
}

var commonArgs = ["-iwad", "doom1.wad", "-window", "-nogui", "-width", "320", "-height", "240", "-nomusic", "-config", "default.cfg", "-servername", "doomflare"];
...
var Module = {
  ...
  noInitialRun: true,
  preRun: () => {
    Module.FS.createDataFile('', 'doom1.wad', base64.toByteArray(doomWad), true, true);
    Module.FS.createDataFile('', 'default.cfg', doomConfig, true, true);
  },
  wasmBinary: base64.toByteArray(wasmBinaryFileBase64),
...
var ABORT = false;
export function setAbort(x) { ABORT = x; }

var EXITSTATUS;
export function setExitStatus(x) { EXITSTATUS = x; }
...
function _emscripten_set_blur_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  return 0; // disable set blur events
...
function _emscripten_set_focus_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  return 0; // disable set focus events
...
function _emscripten_set_fullscreenchange_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  return 0; // disable set full screen events
  if (!JSEvents.fullscreenEnabled()) return -1;
  target = findEventTarget(target);
  if (!target) return -4;
...
function _emscripten_set_pointerlockchange_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  return -1; // disable pointer lock events
...
function _emscripten_set_resize_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  return 0; // disable resize events
...
function _emscripten_set_visibilitychange_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
  return 0; // disable visibility change events
...
function _emscripten_set_wheel_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
  return -1; // disable set wheel events
  target = findEventTarget(target);
